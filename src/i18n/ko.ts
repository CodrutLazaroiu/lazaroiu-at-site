import type { Dictionary } from './types';

// Launch scope per brief Section 4: core pages (Home, Advisory, About, Contact).
// Research stays in English with the translated abstract paragraph below.
// Pending native review before being treated as final.
const ko: Dictionary = {
  locale: 'ko',
  dir: 'ltr',
  siteName: 'Ioan-Codrut Lazaroiu',

  ctaLabel: '대화 시작하기',
  contactLead:
    '오스트리아 빈. 서면 문의에는 영업일 기준 2일 이내에 답변드립니다. 귀하가 누구인지, 어느 조직을 대표하는지, 그리고 직면한 결정이 무엇인지 알려주시면 도움이 됩니다.',
  productCaption:
    '개발 중: SALTUS, Microsoft 365 네이티브 전사적 리스크 거버넌스 플랫폼. 세부 사항은 요청 시 제공.',

  nav: {
    advisory: 'Advisory',
    research: '연구',
    about: '소개',
    contact: '연락처',
  },

  footer: {
    linkedin: 'LinkedIn',
    impressum: 'Legal notice',
    datenschutz: 'Privacy',
  },

  home: {
    metaTitle: 'Ioan-Codrut Lazaroiu — 대체 불가능한 인프라를 위한 전략, 리스크, 자본',
    metaDescription:
      '오스트리아 빈의 독립 에너지 인프라·리스크 거버넌스 어드바이저. 전사적 리스크 거버넌스, 자산 건전성, 국경 간 인프라 전략.',
    eyebrow: '독립 자문 — 오스트리아 빈',
    eyebrowShort: '독립 자문 — 빈',
    h1: '대체 불가능한 인프라를 위한 전략, 리스크, 자본.',
    lede: '전사적 리스크 거버넌스, 자산 건전성, 국경 간 인프라 전략이 교차하는 영역의 시니어 어드바이저. 에너지 인프라 분야 20년의 실행 경험을 바탕으로, 현재는 이사회 수준의 자문과 독립 연구를 수행하고 있습니다.',
    pillars: [
      {
        num: '01',
        title: 'Advisory',
        text: '운영사와 투자자를 위한 전사적 리스크 거버넌스 설계, 전환 프로젝트, 경영진 의사결정 지원.',
        href: '/advisory/',
        cta: '→ /ko/advisory',
      },
      {
        num: '02',
        title: '연구',
        text: '인프라, 자본 배분·리스크, 전환, 그리고 조직과 인간-AI 팀의 행동 예측에 관한 독립 워킹 페이퍼.',
        href: '/research/',
        cta: '→ /ko/research',
      },
      {
        num: '03',
        title: '소개',
        text: '유럽, 중동, 북·서아프리카, 중앙아시아에 걸친 20년의 실행 경험.',
        href: '/about/',
        cta: '→ /ko/about',
      },
    ],
    papers: [
      {
        eyebrow: '워킹 페이퍼 · 2026년 9월',
        title:
          'The Clock, Not the Culture — Predicting Workplace Behavior from the Individual to the Multinational in the Age of AI Colleagues',
        thesis: '“문화가 아니라 의사결정 지연 시간이다.”',
        cta: '초록 및 논문 → /ko/research',
        ctaShort: '초록 + 논문 → /ko/research',
        href: '/research/',
      },
      {
        eyebrow: '워킹 페이퍼 · 2026년 8월',
        title:
          'Molecules of Optionality — Why Europe’s Pipeline Transition Is a Capital-Allocation Problem, Not an Engineering One',
        thesis: '“파이프는 대체할 수 있어도, 회랑은 대체할 수 없다.”',
        cta: '초록 및 논문 → /ko/research',
        ctaShort: '초록 + 논문 → /ko/research',
        href: '/research/',
      },
      {
        eyebrow: '워킹 페이퍼 · 2026년 8월',
        title: 'The Settlement Layer — The Missing Financial Infrastructure of the Circular Economy',
        thesis: '“반환은 미덕이 아니라 정산이다.”',
        cta: '초록 및 논문 → /ko/research',
        ctaShort: '초록 + 논문 → /ko/research',
        href: '/research/',
      },
    ],
  },

  advisory: {
    metaTitle: 'Advisory — Ioan-Codrut Lazaroiu',
    metaDescription:
      '전사적 리스크 거버넌스 설계와 전환, 리스크 문화와 오너십, 경영진 보고, 자산 관리와 프로젝트 수행, 국경 간 거버넌스.',
    eyebrow: 'Advisory',
    h1: 'Advisory',
    lede: '에너지 인프라 운영사, 투자자, 대출기관의 이사회와 경영진만을 위한 엄선된 위임.',
    servicesLabel: '서비스',
    services: [
      {
        num: '01',
        title: '전사적 리스크 거버넌스',
        text: '설계 및 전환 프로젝트: 거버넌스 아키텍처, 리스크 성향, 그리고 이를 의사결정과 연결하는 운영 모델.',
      },
      {
        num: '02',
        title: '리스크 문화와 오너십',
        text: '리스크를 보고 조직이 아니라 자산을 책임지는 경영진에게 두는 오너십 모델.',
      },
      {
        num: '03',
        title: '경영진 보고 및 의사결정 지원',
        text: '의사결정을 위한 이사회·위원회 보고: 더 적은 지표, 더 명확한 임계값, 명시적인 트레이드오프.',
      },
      {
        num: '04',
        title: '자산 관리 및 프로젝트 수행',
        text: '발주자 측 자산 관리와 자본 프로젝트 수행의 전환 — 터미널, 저장 탱크, 정유소, 파이프라인 시스템을 설계부터 턴어라운드까지.',
      },
      {
        num: '05',
        title: '국경 간 거버넌스',
        text: '복수 관할권 운영사를 위한 거버넌스: 규제기관, 주주, 국경을 아울러 하나의 기업으로 전환하고 하나로 관리하는 체계.',
      },
    ],
    trackRecordLabel: '실적',
    trackRecord: [
      '유럽, 중동, 북·서아프리카, 중앙아시아의 에너지 인프라에서 20년간 실행을 담당했으며, 자본 프로젝트 포트폴리오는 85억 유로를 상회합니다.',
      'OMV, Wood Group, Nabucco, ILF Consulting Engineers에서 시니어 직책을 역임했습니다.',
      '현재 수행 중인 주요 프로젝트: 유럽 국경 간 파이프라인 그룹의 전사적 거버넌스·리스크 전환, 그리고 중동과 중앙아시아에서의 자문 프로젝트.',
    ],
    closing: '개발 중: Microsoft 365 네이티브 전사적 리스크 거버넌스 플랫폼. 세부 사항은 문의 바랍니다.',
  },

  researchPdfLabels: ['논문 읽기 (PDF, 67쪽)', '논문 읽기 (PDF, 12쪽)', '논문 읽기 (PDF, 16쪽)'],
  researchSsrnRecordLabels: ['SSRN Abstract ID: 7427178', 'SSRN Abstract ID: 7193238', 'SSRN Abstract ID: 7211658'],

  researchAbstracts: [
    '한 사람의 직장 내 행동을 — 약하고 조건부이지만 측정 가능하게 — 예측할 수 있다면, 팀의 행동은? AI 동료가 포함된 팀은? 기업은? 다국적 기업은? 이 논문은 문헌이 가장 빈약한 수준에서 원자료를 제시한다. 조직이 사람에게 쓰는 도구 — DISC 형식 검사, 빅 파이브 설문, 반복 게임 배터리 — 를 실제 직무가 구성하는 방식대로 구성된 AI 동료에게 적용한 9,772건의 측정이다. 결론은 세 가지다. 조직이 자문가에게 — 인간이든 인공지능이든 — 부여하는 역할은 국가 문화가 아니라 의사결정 지연 시간을 따른다. 문서화된 힘은 예측하지만 문서화되지 않은 힘은 예측하지 못한다. 직무는 AI 동료의 스타일을 쓰고, 모델은 그 행동을 지닌다.',
    '유럽의 파이프라인 회랑은 통상 엔지니어링 자산으로 분석되지만, 이 논문은 이를 네 가지 가능한 분자적 미래에 대한 가격이 잘못 매겨진 옵션으로 다룬다. 인허가와 계약을 갖추고 국경을 넘어 물리적으로 이어진 회랑은 다시 지을 수 없는 자산이며, 그 안의 파이프는 교체할 수 있다. 전환의 결정적 제약은 기술적 타당성이 아니라 수요 리스크다. 따라서 운영사의 핵심 과제는 국경을 넘어 수단을 쌓아 올리는 것 — 용량, 계약, 규제 수단을 정렬하여 회랑의 옵션 가치를 잃지 않고 보존하는 것이다.',
    '유럽의 반환 시스템은 하나의 금융 메커니즘을 입증한다. 미반환에 구속력 있는 결과가 결부된 곳에서는 용기가 85–99퍼센트 회수되고, 그렇지 않은 곳에서는 선의가 실패한다. 빠져 있는 조각은 정산 계층 — 청구가 국경, 품목, 경쟁 시스템을 넘어 이동할 수 있게 하는 정체성·의무·청산의 공유 장부다. 지금 적용되는 규제는 국가별 순환 고리를 연결하지 못한 채 늘리기만 할 것이다. 이 논문은 오늘날 쓰이는 다섯 가지 수단의 가격을 매기고, 정산이 멈추는 세 개의 이음새를 짚으며, 운영사·유통사·투자자를 위한 네 가지 시험 — 순환 고리를 운영하라, 계층에 참여하라, 옵션을 보유하라, 아니면 짓지 마라 — 로 끝맺는다.',
  ],

  about: {
    metaTitle: '소개 — Ioan-Codrut Lazaroiu',
    metaDescription:
      '오스트리아 빈의 에너지 인프라 임원이자 독립 어드바이저. 조직과 인간-AI 팀의 행동 예측에 관한 연구. 유럽, 중동, 북·서아프리카, 중앙아시아에서 20년 이상의 경력.',
    eyebrow: '소개',
    h1: 'Ioan-Codrut Lazaroiu',
    lede: '오스트리아 빈의 에너지 인프라 임원이자 독립 어드바이저. 조직과 인간-AI 팀의 행동 예측에 관한 연구.',
    portraitAlt: 'Ioan-Codrut Lazaroiu',
    portraitCaption: '오스트리아 빈',
    rows: [
      {
        label: '경력',
        text: '유럽, 중동, 북·서아프리카, 중앙아시아의 에너지 인프라 분야에서 20년 이상.',
      },
      {
        label: '포트폴리오',
        text: '85억 유로를 상회하는 자본 프로젝트 포트폴리오 — 터미널, 저장 탱크, 정유소, 국제 파이프라인 시스템의 설계·건설·시운전·턴어라운드.',
      },
      {
        label: '주요 경력',
        text: 'OMV (오스트리아, 루마니아, UAE, 튀니지) · Wood Group · Nabucco · ILF Consulting Engineers.',
      },
      {
        label: '현재',
        text: '유럽 국경 간 파이프라인 그룹의 전사적 거버넌스·리스크 전환, 그리고 중동과 중앙아시아에서의 자문 프로젝트.',
      },
      {
        label: '학력',
        text: '공학, 부쿠레슈티 건설기술대학교 · 경제학 학사, 부쿠레슈티 스피루 하레트 대학교 · MIT Sloan School of Management — Executive Programme in General Management, 15기.',
      },
      {
        label: '자격',
        text: 'Institute of Risk Management · PMI · FIDIC · Lean Six Sigma Green Belt.',
      },
      {
        label: '언어',
        text: '영어 · 이탈리아어 · 독일어 · 루마니아어.',
      },
    ],
  },

  contact: {
    metaTitle: '연락처 — Ioan-Codrut Lazaroiu',
    metaDescription:
      'Ioan-Codrut Lazaroiu 연락처 — 오스트리아 빈의 에너지 인프라 임원이자 독립 어드바이저.',
    eyebrow: '연락처',
    h1: '연락처',
    emailLabel: '이메일',
    linkedinLabel: 'LinkedIn',
  },
};

export default ko;
